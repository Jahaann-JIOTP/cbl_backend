        ...
        $pipeline = [
            ...
            [
                '$project' => [
                    'is_on' => [
                        '$cond' => [
                            'if' => ['$gt' => ['$U_7_EM7_ActivePowerTotal_kW', 1]],
                            'then' => 1,
                            'else' => 0
                        ]
                    ]
...
function calculateEnergyConsumption($collection, $start_date, $end_date) {
    try {
        $firstRecord = $collection->findOne(
            ...
            ['sort' => ['timestamp' => 1], 'projection' => ['U_7_EM7_ActiveEnergyDelivered_Wh' => 1]]
        );
        ...
        if ($firstRecord && $lastRecord) {
            $start_energy = $firstRecord['U_7_EM7_ActiveEnergyDelivered_Wh'] ?? 0;
            $end_energy = $lastRecord['U_7_EM7_ActiveEnergyDelivered_Wh'] ?? 0;
...
try {
    ...
    echo json_encode([
        ...
        'energy_consumption_kWh' => round($energy_consumption,2) // Include energy consumption
    ]);
} catch (Exception $e) {
