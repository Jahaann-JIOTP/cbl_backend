        ...
        $pipeline = [
            ...
            [
                '$project' => [
                    'is_on' => [
                        '$cond' => [
                            'if' => ['$gt' => ['$U_10_EM10_Activepower_Total_W', 1]],
                            'then' => 1,
                            'else' => 0
                        ]
                    ]
...
function calculateEnergyConsumption($collection, $start_date, $end_date) {
    try {
        $firstRecord = $collection->findOne(
            ...
            ['sort' => ['timestamp' => 1], 'projection' => ['U_10_EM10_TotalActiveEnergy_kWh' => 1]]
        );
        ...
        if ($firstRecord && $lastRecord) {
            $start_energy = $firstRecord['U_10_EM10_TotalActiveEnergy_kWh'] ?? 0;
            $end_energy = $lastRecord['U_10_EM10_TotalActiveEnergy_kWh'] ?? 0;
